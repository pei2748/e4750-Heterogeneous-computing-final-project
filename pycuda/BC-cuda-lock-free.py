...
dim0 = 1024
block_dim = (dim0, 1, 1)
grid_dim = (math.ceil(num_examples/dim0), 1, 1) #how many examples ??
...
prg_sgd = SourceModule(open("../kernels/sgd_kernel_no_locks.cu", "r").read())

s = time.time()
# Transfer variables onto device
d_x = gpuarray.to_gpu(X_train.to_numpy().astype(np.float32));
d_y = gpuarray.to_gpu(y_train.to_numpy().astype(np.float32));
...
func = prg_sgd.get_function("sgd_lock_free_naive")
